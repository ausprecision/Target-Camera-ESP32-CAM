...
void cmd_type(ESPTelnet &telnet, String str) {
  String tmpString = "/" + str.substring(5); tmpString.trim(); tmpString.toUpperCase();
  File readFile = SPIFFS.open(tmpString, "r");
  if (!readFile) { telnet.print("File not found - "); telnet.println(str.substring(5)); telnet.println(""); telnet.print("C:\\>"); return;  }
  else{ while (readFile.available()) { String line = readFile.readStringUntil('\n'); telnet.println(line);  } readFile.close(); telnet.println(""); telnet.print("C:\\>");  }
}
...
void cmd_dir(ESPTelnet &telnet) {
  ...
  if(root.isDirectory())
  {
    ...
    while(file)
    {
      ...
      telnet.print(string_asukiaaa::padStart(String(csi(fsize)), 7, ' '));
    ...
    telnet.println(string_asukiaaa::padStart(String(csi(fileTotalBytes)), 15, ' ') + " bytes");
    telnet.println(string_asukiaaa::padStart(String(csi(SPIFFS.totalBytes() - SPIFFS.usedBytes())), 32, ' ') + " bytes free");
