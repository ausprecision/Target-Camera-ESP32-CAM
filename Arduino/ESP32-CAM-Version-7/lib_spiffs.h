...
void spiffs_files_initialization(){
  ...
  if (!SPIFFS.exists("/NETWORKS.TXT")) { Serial.println("spiffs_files: /NETWORKS.TXT does not exist, creating file"); file = SPIFFS.open("/NETWORKS.TXT", FILE_WRITE); file.print(txtNetworks);             file.close(); }
